...
// COVID over time and race

clear
insheet using https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv //NYTimes Github data, names
keep if state == "California" | state == "Illinois" | state == "New Jersey"
keep if county == "Cook" | county == "Los Angeles" | county == "Essex"
drop state
drop fips
save COVID_CA_IL_NJ.dta, replace

clear
insheet using https://raw.githubusercontent.com/jamesonrutgers/Raw-data/main/ACSDT5Y2019.B02001_data_with_overlays_2021-05-05T195019.csv
drop v1
rename v2 county
replace county = subinstr(county,"County, California","", .)
replace county = subinstr(county,"County, Illinois","", .)
replace county = subinstr(county,"County, New Jersey","", .)
replace county = subinstr(county," ","", .)
replace county = "Los Angeles" in 3
drop v20 v21 v22
keep county* v3* v5* v7* v9* v11*
rename v3 pop
rename v5 white
rename v7 black
rename v9 indigenous
rename v11 asian
save RACE_CA_IL_NJ.dta, replace

clear
use COVID_CA_IL_NJ.dta
merge m:1 county using RACE_CA_IL_NJ.dta
drop _merge
save RACE_COV_CA_IL_NJ.dta, replace

clear
use RACE_COV_CA_IL_NJ.dta
destring black, gen(black2) force
graph hbar cases deaths black2, over(county)

line cases date

gen date2 = date(date,"YYYY-MM-DD", 2020) //dates are in string format, trying to destring them
// doing a simple --- destring date, gen(date2) force --- does not work
format date2 %td //trying to make this work but cannot
